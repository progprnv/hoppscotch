...
export class RESTRequestExecutor {
  ...
  private normalizeParams(params: HoppRESTParams): Record<string, string> {
    ...
    this.logger.debug("[RESTRequestExecutor] Normalized params result:", normalizedParams)
  ...
  private normalizeHeaders(
    ...
    this.logger.debug("[RESTRequestExecutor] Normalized headers result:", normalizedHeaders)
  ...
  private async constructKernelRequest(
    ...
    this.logger.info("[RESTRequestExecutor] Constructing kernel request for URL:", req.effectiveFinalURL)
    ...
    try {
      ...
      this.logger.debug("[RESTRequestExecutor] Constructed kernel request:", kernelRequest)
      return kernelRequest
    } catch (error) {
      this.logger.error("[RESTRequestExecutor] Error constructing kernel request:", error)
      throw error
    }
  }

  private async constructRequestContent(body: string | FormData | null | File) {
    this.logger.debug("[RESTRequestExecutor] Constructing request content, type:", body ? typeof body : 'null')

    try {
      if (!body) {
        this.logger.debug("[RESTRequestExecutor] No body provided, returning undefined")
        return undefined
      }
      ...
      if (body instanceof File) {
        this.logger.debug("[RESTRequestExecutor] Processing File body:", body.name, body.type)
      ...
      this.logger.error("[RESTRequestExecutor] Error constructing request content:", error)
      throw error
    }
  }
  ...
  private extractHeaders(
    ...
    this.logger.debug("[RESTRequestExecutor] Extracting headers from record:", headers)
    ...
    this.logger.debug("[RESTRequestExecutor] Extracted headers result:", extractedHeaders)
    return extractedHeaders
  }

  private getResponseBody(res: Response): ArrayBuffer {
    this.logger.debug("[RESTRequestExecutor] Getting response body for content kind:", res.content.kind)
    ...
    try {
      ...
      switch (res.content.kind) {
        ...
        case "text":
          ...
          break
        case "json":
          result = new TextEncoder().encode(JSON.stringify(res.content.content)).buffer
          break
        default:
          this.logger.warn("[RESTRequestExecutor] Unknown content kind:", res.content.kind)
          result = new ArrayBuffer(0)
      }

      this.logger.debug("[RESTRequestExecutor] Response body size:", result.byteLength, "bytes")
      return result
    } catch (error) {
      this.logger.error("[RESTRequestExecutor] Error processing response body:", error)
      throw error
    }
  }
  ...
  private createResponseFromKernel(
    ...
    this.logger.info("[RESTRequestExecutor] Creating response from kernel, status:", res.status)
    ...
    try {
      ...
      this.logger.error("[RESTRequestExecutor] Error creating response from kernel:", error)
      throw error
    }
  }
  ...
  public async execute(
    ...
    try {
      ...
      kernelResponse.then((result) => {
        if (E.isRight(result)) {
          this.logger.info("[RESTRequestExecutor] Request completed successfully")
          response.next(this.createResponseFromKernel(result.right, request))
        } else {
          this.logger.error("[RESTRequestExecutor] Request failed:", result.left)
          response.next({
            type: "network_fail",
            error: result.left,
            req: request,
...
export async function createRESTNetworkRequestStream(
  ...
  console.info("[RESTNetworkRequest] Creating request stream for:", request.effectiveFinalURL)
